...
// On définit la position de la section
const sectionPosition = boutiqueSection.getBoundingClientRect().top;

// On ajoute un événement de défilement
window.addEventListener('scroll', () => {
  // On calcule la position de la section par rapport à la fenêtre
  const sectionInView = sectionPosition - windowHeight <= 0;

  // On ajoute la classe "in-view" si la section est visible
  if (sectionInView) {
    boutiqueSection.classList.add('in-view');
  }
});
</script>
<?php
